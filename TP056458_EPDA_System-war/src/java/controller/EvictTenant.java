 ...
 * @author dzaky
 */
@WebServlet(name = "EvictTenant", urlPatterns = {"/EvictTenant"})
public class EvictTenant extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (property == null) {
                ...
                try {
                    ...
                    if (property.getRenterID() == null) {
                        throw new Exception();
                    }
                    property.setRentedBy(null);
                    property.setRenterID(null);
                    myPropertyFacade.edit(property);
                    request.getRequestDispatcher("ManageProperty.jsp").include(request, response);
                    out.println("<div id=\"container\">");
                    out.println("Property ID " + listingID + "'s tenant has been evicted!");
                    out.println("</div>");
                } catch (Exception e) {
                    request.getRequestDispatcher("ManageProperty.jsp").include(request, response);
                    out.println("<div id=\"container\">");
