...
public class Withdraw extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try{
                double y = Double.parseDouble(withdraw);
                if(y<=0){
                    throw new Exception();
                }
                if(y>user.getBalance()){
                    throw new Exception();
                }
                double withdrawAmount = user.getBalance() - y;
                user.setBalance(withdrawAmount);
                myUserFacade.edit(user);
                request.getRequestDispatcher("Wallet.jsp").include(request, response);
                out.println("<div id=\"container\">");
                out.println("Amount of " + withdraw + " has been withdrawn!");
                out.println("</div>");
